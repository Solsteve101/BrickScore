  ...
  return (
    <button
      ...
      className={cn(
        ...
        isDark
          ? "bg-white text-[#1C1C1C] border-gray-200 hover:bg-gray-50"
          : "bg-[#1C1C1C] text-white border-[#1C1C1C] hover:bg-[#2a2a2a]",
        className,
      )}
    >
...
const SolarSwitch = ({ isDark }: { isDark: boolean }) => {
  ...
  useEffect(() => {
    const m = animate(scaleMoon, isDark ? 1 : 0, { duration });
    const s = animate(scaleSun, isDark ? 0 : 1, { duration });
    return () => { m.stop(); s.stop(); };
  }, [isDark, scaleMoon, scaleSun]);

  return (
    <motion.div animate={isDark ? "checked" : "unchecked"}>
      <motion.svg width="14" height="14" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
